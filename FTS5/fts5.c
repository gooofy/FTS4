...
int main(int argc, char **argv)
{
   ...
   log (LOG_INFO, "FTS v5.0 (C) 2019 by G. Bartsch\n\n");

   parse_commandline(argc, argv);

   log (LOG_INFO, "Opening the serial device...\n");
   ...
   serial_open = !OpenDevice("serial.device", 0, (struct IORequest*)io_serial, 0);
   if (!serial_open)
   {
      log (LOG_ERROR, "ERROR: Serial.device did not open.\n");
      closedown();
   }
